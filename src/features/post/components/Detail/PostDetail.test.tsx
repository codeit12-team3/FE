...
jest.mock('./PostManage/PostManage', () => {
  const MockPostManage = ({ postId }: { postId: string }) => {
    const { usePostManage } = jest.requireMock('@/features/post/hooks')
...
jest.mock('./PostHeader/PostHeader', () => {
  const MockPostHeader = ({
    ...
    const { usePostDetail } = jest.requireMock('@/features/post/api')
    const { useBookmarkToggle } = jest.requireMock('@/features/post/hooks')
    ...
    const PostManage = jest.requireMock('./PostManage/PostManage').default
...
jest.mock('./ApplyModal/ApplyModal', () => ({
  __esModule: true,
  default: ({
    onClose,
    onSubmit,
...
jest.mock('@/features/comment/components/CommentContainer', () => ({
  __esModule: true,
...
beforeEach(() => {
  ...
  const { useBookmarkToggle, useApply, usePostManage, useCompanionId } =
    jest.requireMock('@/features/post/hooks')
...
describe('게시글 권한 테스트', () => {
  ...
  describe('내가 작성자가 아닐 경우', () => {
    ...
    test('신청하기 버튼을 누르면 동행이 신청된다.', async () => {
      ...
      const { useApply } = jest.requireMock('@/features/post/hooks')
    ...
    test('신청 취소 버튼을 누르면 동행 신청이 취소된다', async () => {
      ...
      const { useCompanionId } = jest.requireMock('@/features/post/hooks')
